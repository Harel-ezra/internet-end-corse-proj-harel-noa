...
int send_icmp()
{
  ...
  char data[IP_MAXPACKET] = "This is the ping request.\n";
                ...
                const u_char *packet)
{
  ...
  { // 0x0800 is IP type
    ...
    /* determine protocol */
    if ((ip->iph_protocol == IPPROTO_ICMP) &&(strcmp(DESTINATION_IP,inet_ntoa(ip->iph_destip))))
    {
      printf("got an ICMP replay..\n");
      printf("   From: %s\n", inet_ntoa(ip->iph_sourceip));
      printf("   To: %s\n", inet_ntoa(ip->iph_destip));
      // char *data = (u_char *)(packet + sizeof(struct ethheader) + sizeof(struct ipheader) + sizeof(struct icmp));
      // printf("        data:");
